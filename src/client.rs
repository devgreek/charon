...
use crate::protocol::{
    AUTH_NONE, CMD_CONNECT, SOCKS_VERSION, SocksAddr, 
...
impl Client {
    ...
    pub fn with_auth(proxy_addr: String, proxy_port: u16, username: String, password: String) -> Self {
        Client {
            proxy_addr,
            proxy_port,
    ...
    pub async fn connect_to_target<A: ToSocketAddrs>(&self, target_addr: A) -> io::Result<TcpStream> {
    ...
    async fn connect(&self, target: SocksAddr) -> io::Result<TcpStream> {
        ...
        let mut stream = TcpStream::connect(format!("{}:{}", self.proxy_addr, self.proxy_port)).await?;
        debug!("Connected to SOCKS5 proxy {}:{}", self.proxy_addr, self.proxy_port);
    ...
    async fn handshake<T>(&self, stream: &mut T) -> io::Result<()>
    where
        T: AsyncReadExt + AsyncWriteExt + Unpin,
    {
        use crate::protocol::{AUTH_PASSWORD, UserPassAuth, AUTH_SUCCESS, AUTH_VERSION};
        ...
        if response[0] != SOCKS_VERSION {
            return Err(io::Error::new(io::ErrorKind::Other, "Invalid SOCKS version from proxy"));
        }
            ...
            AUTH_PASSWORD => {
                if let Some((username, password)) = &self.auth {
                    ...
                    if auth_response[0] != AUTH_VERSION {
                        return Err(io::Error::new(io::ErrorKind::Other, "Invalid auth protocol version"));
                    }
                    ...
                    Err(io::Error::new(io::ErrorKind::Other, "Server requested auth but no credentials provided"))
                }
            },
            0xFF => {
                error!("No acceptable authentication methods");
                Err(io::Error::new(io::ErrorKind::Other, "No acceptable authentication methods"))
            },
            _ => {
                error!("Unknown authentication method: {}", response[1]);
                Err(io::Error::new(io::ErrorKind::Other, format!("Unknown authentication method: {}", response[1])))
            }
        }
    }

    async fn request_connection<T>(&self, stream: &mut T, addr: SocksAddr) -> io::Result<()>
    where
        T: AsyncReadExt + AsyncWrite + Unpin,
    {
        ...
        if version != SOCKS_VERSION {
            return Err(io::Error::new(io::ErrorKind::Other, "Invalid protocol version in response"));
        }
        ...
        match addr_type {
            ...
            _ => {
                return Err(io::Error::new(io::ErrorKind::Other, "Invalid address type in response"));
            }
        }
